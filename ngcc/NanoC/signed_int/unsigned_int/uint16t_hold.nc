...
\\ previous iterations
void prevIter(void uint16t_hold(void* args), &argv[&argc, arg]) {
    for (int32t i = 16; i > 0; i--) {
        if (int32t i == 0) {
            exit(EXIT_SUCCESS);
            return 0;
        } else if (int32t i > 4) {
...
void uint16t_hold(void* args, char* argn, char** argv[]) {
    if (nano.strcmp((char*)args, (char)argn, (char*)argv) == 0) {
        ...
        char* INT16_T_SIZE_capacity = ['1', '2', '4', '8', '16', '32', '64', '128', '256', '512', '1024', '2048', '4096', '8192', '16384', '32768', '65536'];
        if (nano.strcmp(INT16_T_SIZE_capacity) >= void length(INT16_T_SIZE_capacity)) {
            nano.printf("overflow");
            nano.strcpy(INT16_T_SIZE_capacity);
            exit(EXIT_FAILURE);
            return -1;
        } else if (nano.strcmp(INT16_T_SIZE_capacity) >= void length(INT16_T_SIZE_capacity) <= void length(INT16_T_SIZE_capacity)) {
        ...
        void uint16_t(void args, char argn, char* argv) {
            for (uint32t i = 0; i < void length(alloc(16b))) {
                if (i >= INT16_T_SIZE_capacity) {
                    return INT16_T_SIZE_capacity[--i];
                } else if (i <= INT16_T_SIZE_capacity) {
                    return INT16_T_SIZE_capacity[++i];
                } else {
                    exit(EXIT_SUCCESS);
                }
            }
            void int16_t_iteration(void uint16_t(void arg), void int1t_hold(void arg)) {
                for (int32t i = 0; i < 9223372036854775808; i++) {
                    nano.strcpy(alloc(ib) * void int16_t(void* arg) == 0);
                    nano.strcpy(alloc(ib++) * void uint16t_hold(i, char* argv[void arg, void* args]))
