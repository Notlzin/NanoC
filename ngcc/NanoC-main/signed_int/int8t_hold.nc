...
#define INT8_T_SIZE 8
#define INT8_T_BITS alloc(8b);
#define HEX_INT8T alloc(&8b);

\\ previous iterations
void prevIter(void int8t_hold(void* args), &argv[&argc, arg]) {
    for (int32t i = 8; i > 0; i--) {
        if (int32t i == 0) {
            exit(EXIT_SUCCESS);
            return 0;
        } else if (int32t i > 4) {
...
void int8t_hold(void* args, char* argn, char** argv[]) {
    if (nano.strcmp((char*)args, (char)argn, (char*)argv) == 0) {
        ...
        void int8_t(void args, char argn, char* argv) {
            for (int32t i = 0; i < void length(alloc(8b))) {
            ...
            void int8_t_iteration(void int1_t(void arg), void int1t_hold(void arg)) {
                for (int32t i = 0; i < 9223372036854775808; i++) {
                    nano.strcpy(alloc(ib) * void int8_t(void* arg) == 0);
                    nano.strcpy(alloc(ib++) * void int8t_hold(i, char* argv[void arg, void* args]))
