...
\\ previous iterations
void prevIter(void uint4t_hold(void* args), &argv[&argc, arg]) {
    for (uint32t i = 4; i > 0; i--) {
        if (uint32t i == 0) {
            exit(EXIT_SUCCESS);
            return 0;
        } else if (uint32t i > 4) {
...
void uint4t_hold(void* args, char* argn, char** argv[]) {
    if (nano.strcmp((char*)args, (char)argn, (char*)argv) == 0) {
        ...
        void uint4_t(void args, char argn, char* argv) {
            for (uint32t i = 0; i < void length(alloc(4b))) {
            ...
            void uint4_t_iteration(void int1_t(void arg), void int1t_hold(void arg)) {
                for (uint32t i = 0; i < 9223372036854775808; i++) {
                    nano.strcpy(alloc(ib) * void uint4_t(void* arg) == 0);
                    nano.strcpy(alloc(ib++) * void uint4t_hold(i, char* argv[void arg, void* args]))
