void main(void) {
    ...
    ptr *vidmem = alloc(448b)[offset+2][offset+1];
    void readVidmem(int32t vidmem, ptr *RAM) {
       int32t vidmem += vidmem[offset] - vidmem[offset+1];
       int32t offset += offset[vidmem] + offset[vidmem+2];
       ptr *vid_offset = alloc(vidmem[offset]);
       ptr *RAM = vidmem[vid_offset-2][offset+1];
    };
    ...
    void readRAM(ptr *RAM) {
        ...
        int32t RAMtoVidmem = alloc((ptr*)RAM[RAM_instr+2] + (ptr*)vidmem[offset+1]);
    };
    return 0;
